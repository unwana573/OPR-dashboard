...
function PricingStrategy() {
  ...
  const handlePriceChange = (screenIndex, showIndex, priceIndex, value) => {
    const newData = [...pricingData];
    newData[screenIndex].prices[showIndex][priceIndex] = value;
            ...
            <table className="w-full">
              <thead>
                <tr className="border-b">
                  <th className="text-left py-3 px-4 text-sm font-medium text-gray-700 bg-gray-50">
                    Screens
                  </th>
                  {shows.map((show) => (
              ...
              <tbody>
                {pricingData.map((row, screenIndex) => (
                  <tr key={screenIndex} className="border-b">
                    <td className="py-3 px-4 text-sm font-medium text-gray-700 align-top">
                      <div className="font-semibold">{row.screen}</div>
                      <div className="text-xs text-gray-500 mt-1">{row.movie}</div>
                    </td>
                    {row.prices.map((showPrices, showIndex) => (
                      <td key={showIndex} className="py-3 px-4">
                        <div className="flex flex-col gap-1">
                          {showPrices.map((price, priceIndex) => (
                            <div key={priceIndex}>
                              {isEditing ? (
                                <input
                                  type="text"
                                  value={price}
                                  onChange={(e) => handlePriceChange(screenIndex, showIndex, priceIndex, e.target.value)}
